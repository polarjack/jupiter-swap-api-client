...
async fn main() -> Result<()> {
    ...
    // Create client with optional API key from environment variable
    let jupiter_swap_api_client = match env::var("JUPITER_API_KEY") {
        Ok(api_key) => {
            println!("Using provided API key {} for authenticated requests", api_key);
            JupiterSwapApiClient::new_with_api_key(api_base_url, api_key)
        }
        Err(_) => {
            println!("No API key provided, using client without authentication");
            JupiterSwapApiClient::new(api_base_url)
        }
    };
