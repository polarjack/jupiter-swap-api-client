...
async fn main() -> Result<()> {
    ...
    // Determine the Jupiter API base URL, falling back to the standard endpoint.
    let api_base_url =
        env::var("API_BASE_URL").unwrap_or_else(|_| "https://lite-api.jup.ag/swap/v1".into());
    println!("Using Jupiter base url: {}", api_base_url);
    ...
    // Print full quote request in the URL format
    let internal_quote_request =
        jupiter_swap_api_client::quote::InternalQuoteRequest::from(quote_request.clone());
    let query_string = serde_qs::to_string(&internal_quote_request)?;
    println!(
        "Quote Request URL: {}/quote?{}",
        env::var("API_BASE_URL").unwrap_or_else(|_| "https://lite-api.jup.ag/swap/v1".into()),
        query_string
    );
